...
function checkIfCourseConflicts(selectedClass) {
    ...
    const selectedClassStartTime = selectedClass.times.substr(0, selectedClass.times.indexOf("-"));
    const selectedClassEndTime = selectedClass.times.split('-')[1];

    //convert the start and end times to military time for easier comparison
    const newSelectedClassStartTime = convertToMilitaryTime(selectedClassStartTime);
    const newSelectedClassEndTime = convertToMilitaryTime(selectedClassEndTime);
    ...
    for (var i = 0; i < mainCourses.length; i++) {
        ...
        console.log(comparingClass.days.length);
        ...
        if (sameDay == true) {
            console.log("course needs to be checked further");
        } else {
...
function createPrintButton() {
    ...
    if (printBtn == null) createIt();else {
        removeIt();
        createIt();
    }
...
function printFunc() {
    ...
    document.body.innerHTML = "<html><head><title></title></head><body>" + printDiv + "<br></br><span>Advisor:_______________________________________   Date:________</span></body>";
